import mobilData from '../../../public/CarShowroom/car.js';
import{ useState } from 'react';
import '../../../src/component/CarShoowroom/Filter.css';
...
function Mobil() {
  ...
  {filteredMobil.length === 0 ? (
    ...
    filteredMobil.map((mobil) => (
      <div className='hasil' key={mobil.id}> {}
        <img className='gambar' src={mobil.path} alt={mobil.nama} />
        <div className='text'>
          <h1 style={{fontWeight: 'bold', fontSize: '25px', lineHeight: '24px', color: 'black'}}>{mobil.nama}</h1>
          <div className='pil-text'>
            <p className='text-paragraf'>Merek : {mobil.merek}</p> {}
            <p className='text-paragraf'>Warna : {mobil.warna}</p>
            <p className='text-paragraf'>Tipe : {mobil.tipe}</p>
          </div>
        </div>
